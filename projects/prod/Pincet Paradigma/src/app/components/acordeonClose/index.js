...
function detOpen(acordeon) {
  for (let i = 0; i < acordeon.length; i++) {
    acordeon[i].addEventListener("click", function (e) {
      ...
      if (
        this.parentElement.classList.contains("cosmetic__catalog-accordeon")
      ) {
        openClass = "showDet3";
        paddingClass = "pad2";
      }
...
function detOpen(acordeon) {
  for (let i = 0; i < acordeon.length; i++) {
    acordeon[i].addEventListener("click", function (e) {
      ...
      if (this.classList.contains(openClass)) {
        this.classList.toggle(openClass);
        this.nextElementSibling.classList.toggle(paddingClass);
        panel.style.maxHeight = null;
      } else {
        this.classList.add(openClass);
        this.nextElementSibling.classList.add(paddingClass);
        panel.style.maxHeight = panel.scrollHeight + 20 + "px";
        if (last) {
          let panels = last.nextElementSibling;
          panels.style.maxHeight = null;
          last.classList.toggle(openClass, false);
          last.nextElementSibling.classList.toggle(paddingClass, false);
        }
        last = this;
        if (last.classList.contains(openClass) === false) {
          last.classList.add(openClass);
          last.nextElementSibling.classList.add(paddingClass);
          panel.style.maxHeight = panel.scrollHeight + 20 + "px";
        }
      }
    });
