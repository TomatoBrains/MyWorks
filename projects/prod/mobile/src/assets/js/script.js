...
$('.btn-zakaz-1-js').on('click', function(event) {
  ...
  var tlt = $(this).parents('.shtory-right').find('.tlt-one-js').text().trim();
  $('.tlt-js').text(tlt);
  $('.inp-js').val(tlt);
  $('#modal-zakaz').fadeIn();
});
...
$('.read').on('click', function(event) {
  ...
  if(!$(this).hasClass('show')){
    ...
    $(this).removeClass('show').text('Читать далее');
     $(this).parents('.shtory-right-one').find('li').each(function(index, el) {
       if(index > 2){
        $(this).hide();
       }
     });
...
$('.shtory-tab__item').on('click', function (event) {
  ...
  $(window).scroll(function(event) {
    ...
    if($(window).scrollTop() > darkSection && $(window).scrollTop() < darkSection2 + $('.zakaz').outerHeight() ){
