...
const APP_ENV = argv.env.mode === "production" ? "production" : "development";
...
pages.forEach((page) => {
  htmlPagesGenerated.push(
    new HtmlWebpackPlugin({
      template: page,
      filename: path
        .relative(path.join(dirs.src, "pages"), page)
        .replace(".handlebars", ".html"),
      publicPath: dirs.src,
    })
  );
});
          ...
          use: [
            ...
            },
            {
              ...
              options: {
                ...
                includePaths: [
                  path.resolve(__dirname, path.join(".", dirs.src)),
                ],
              },
            },
          ],
  ...
  resolve: {
    ...
    alias: {
      Utilities: path.resolve(
        __dirname,
        path.join(dirs.src, "app", "utilities")
      ),
    },
  },
  resolveLoader: {
